...
class GbizinfoClient {
  ...
  /**
   * {@link https://info.gbiz.go.jp/hojin/swagger-ui.html#!/gBizINFO_REST_API/getProcurementUsingGET|APIドキュメント}
   * @param {string} corporateNumber 法人番号
   * @return {Promise<Array>} 調達情報
   */
  findProcurementByCorporateNumber (corporateNumber) {
    return this.findProcurementByCorporateNumberRaw(corporateNumber)
      .then((data) => {
        const corporations = data['hojin-infos']
        return corporations.map(convertToCorporate)
      })
   ...
   * @return {unknown} レスポンスボディ
   */
  findProcurementByCorporateNumberRaw (corporateNumber) {
    return request(`${API_ENDPOINT}/v1/hojin/${corporateNumber}/procurement`, { headers: this.defaultHeaders })
      .then(({ body }) => body.json())
  }

  /**
   * {@link https://info.gbiz.go.jp/hojin/swagger-ui.html#!/gBizINFO_REST_API/getSubsidyUsingGET|APIドキュメント}
   * @param {string} corporateNumber 法人番号
   * @return {Promise<Array>} 補助金情報
   */
  findSubsidyByCorporateNumber (corporateNumber) {
    return this.findSubsidyByCorporateNumberRaw(corporateNumber)
      .then((data) => {
        const corporations = data['hojin-infos']
        return corporations.map(convertToCorporate)
      })
   ...
   * @return {unknown} レスポンスボディ
   */
  findSubsidyByCorporateNumberRaw (corporateNumber) {
    return request(`${API_ENDPOINT}/v1/hojin/${corporateNumber}/subsidy`, { headers: this.defaultHeaders })
      .then(({ body }) => body.json())
  }

  /**
   * {@link https://info.gbiz.go.jp/hojin/swagger-ui.html#!/gBizINFO_REST_API/getWorkplaceInformationUsingGET|APIドキュメント}
   * @param {string} corporateNumber 法人番号
   * @return {Promise<Array>} 職場情報
   */
  findWorkplaceByCorporateNumber (corporateNumber) {
    return this.findWorkplaceByCorporateNumberRaw(corporateNumber)
      .then((data) => {
        const corporations = data['hojin-infos']
        return corporations.map(convertToCorporate)
      })
   ...
   * @return {unknown} レスポンスボディ
   */
  findWorkplaceByCorporateNumberRaw (corporateNumber) {
    return request(`${API_ENDPOINT}/v1/hojin/${corporateNumber}/workplace`, { headers: this.defaultHeaders })
      .then(({ body }) => body.json())
  }
  ...
  findByTimeRangeRaw (page, from, to) {
    return request(`${API_ENDPOINT}/v1/hojin/updateInfo`, {
      ...
      .then(timeRangeApiResponseHandler)
  }
