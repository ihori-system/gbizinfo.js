...
describe('findPatentByCorporateNumber', () => {
  test('find patent by corporate number', async () => {
    const client = new GbizinfoClient({ token: process.env.X_HOJININFO_API_TOKEN })
    const actual = await client.findPatentByCorporateNumber('8000012010038') // デジタル庁
    expect(actual.length).toEqual(1)
    expect(actual[0].corporateNumber).toEqual('8000012010038')
    expect(actual[0].patent).toBeDefined()
  })
})
...
describe('findProcurementByCorporateNumber', () => {
  test('find procurement by corporate number', async () => {
    const client = new GbizinfoClient({ token: process.env.X_HOJININFO_API_TOKEN })
    const actual = await client.findProcurementByCorporateNumber('8000012010038') // デジタル庁
    expect(actual.length).toEqual(1)
    expect(actual[0].corporateNumber).toEqual('8000012010038')
    expect(actual[0].procurement).toBeDefined()
  })
})
...
describe('findWorkplaceByCorporateNumber', () => {
  test('find workplace by corporate number', async () => {
    const client = new GbizinfoClient({ token: process.env.X_HOJININFO_API_TOKEN })
    const actual = await client.findWorkplaceByCorporateNumber('8000012010038') // デジタル庁
    expect(actual.length).toEqual(1)
    expect(actual[0].corporateNumber).toEqual('8000012010038')
...
describe('findByTimeRange', () => {
  test('find by time range', async () => {
    ...
    expect(actual.totalCount).toEqual(1680)
...
describe('findWorkplaceByTimeRange', () => {
  test('find workplace by time range', async () => {
    ...
    expect(actual.totalCount).toEqual(9)
    expect(actual.totalPage).toEqual(1)
    expect(actual.pageNumber).toEqual(1)
    expect(actual.corporations.length).toEqual(9)
